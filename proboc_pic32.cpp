...
std::string IntegralRenderDefinitions(const std::string &message_name,
  ...
  std::string get_fn =
      ...
      " from " + message_name + " at address %p, but message dows not have the "
                                "field \\n\", (void*)msg);\nreturn -1;\n}\n}\n";
  std::string set_fn = "void " + message_name + "_set_" + member_name + "(" +
...
std::string Int32Member::UnitTest() {
  std::string retval = std::string("void ") + message_name + "_" + identifier +
                       "_test(){\n";
...
static void set_field_present()" + identifier + R"(*msg, size_t i) {
  size_t byte_index = i / CHAR_BIT;
...
std::string StringMember::UnitTest() {
  std::string retval = "void " + message_name + "_" + identifier + "_test() {\n";
  retval += message_name + " my_message;\n";
  retval += message_name + "_init(&my_message);\n";
...
static uint16_t htons(uint16_t hostshort) {
  ...
  retval += "int main() {\n";
  retval += "DRV_USART0_Initialize();\n";
  for (const auto& x : members) {
    retval += identifier + "_" + x.second->GetIdentifier() + "_test();\n";
  }
  retval += "PRINT(\"Tests completed\")"; 
  retval += "}\n";
  return retval;
}
